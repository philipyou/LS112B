...
//生成CRC表
/*static void init_crc_table(unsigned int *crc_table)
{
	unsigned int c;
	unsigned int i, j;
...
#define CRC32_ADDR	0x8000
u32_t cal_crc32_multiple(u32_t crc, u8_t *buffer, u32_t size)
{
    u32_t i;
    for (i = 0; i < size; ++i)
    {
    	SPIFlashRead((u8_t *)&crc_table, CRC32_ADDR + (((crc ^ buffer[i]) & 0xff)*4), sizeof(crc_table));
        crc = crc_table ^ (crc >> 8);
    }
